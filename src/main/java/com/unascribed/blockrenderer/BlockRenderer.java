...
public class BlockRenderer {
  ...
  public void onFrameStart(RenderTickEvent e) {
    /**
     * Quick primer: OpenGL is double-buffered. This means, where we draw to is /not/ on the screen. As such, we are free to do whatever we like before
  ...
  private void bulkRender(String modidSpec, int size) {
    ...
    for (ItemStack is : toRender) {
      ...
      if (Minecraft.getSystemTime() - lastUpdate > 33) {
        ...
        renderLoading(I18n.format("gui.rendering", toRender.size(), joined),
            I18n.format("gui.progress", rendered, toRender.size(), (toRender.size() - rendered)), is, (float) rendered / toRender.size());
    ...
    if (rendered >= toRender.size()) {
      ...
      renderLoading(I18n.format("gui.renderCancelled"), I18n.format("gui.progress", rendered, toRender.size(), (toRender.size() - rendered)), null,
  ...
  private void renderLoading(String title, String subtitle, ItemStack is, float progress) {
    ...
    Rendering.drawRect(res.getScaledWidth() / 2 - 50, res.getScaledHeight() / 2 - 1, (res.getScaledWidth() / 2 - 50) + (int) (progress * 100),
        res.getScaledHeight() / 2 + 1, 0xFF55FF55);
    GlStateManager.pushMatrix();
    ...
    mc.updateDisplay();
    /*
     * While OpenGL itself is double-buffered, Minecraft is actually *triple*-buffered. This is to allow shaders to work, as shaders are only available in
     * "modern" GL. Minecraft uses "legacy" GL, so it renders using a separate GL context to this third buffer, which is then flipped to the back buffer with
     * this call.
     */
  ...
  private String render(ItemStack is, File folder, boolean includeDateInFilename) {
    ...
    for (String prefix : new String[] { "^enderio_", "^tile_", "^block_", "^item_", "^enderio_", "^tile_", "^block_", "^item_" }) {
    ...
    mc.getRenderItem().renderItemAndEffectIntoGUI(is, 0, big ? 8 : 0);
    GlStateManager.popMatrix();
    try {
      /*
       * We need to flip the image over here, because again, GL Y-zero is the bottom, so it's "Y-up". Minecraft's Y-zero is the top, so it's "Y-down". Since
       * readPixels is Y-up, our Y-down render is flipped. It's easier to do this operation on the resulting image than to do it with GL transforms. Not faster,
       * just easier.
       */
  ...
  private void setUpRenderState(int desiredSize) {
    ...
    /*
     * As we render to the back-buffer, we need to cap our render size to be within the window's bounds. If we didn't do this, the results of our readPixels up
    ...
    /*
     * The GUI scale affects us due to the call to setupOverlayRendering above. As such, we need to counteract this to always get a 512x512 render. We could
     * manually switch to orthogonal mode, but it's just more convenient to leverage setupOverlayRendering.
     */
  ...
  public BufferedImage readPixels(int width, int height) throws InterruptedException {
    /*
     * Make sure we're reading from the back buffer, not the front buffer. The front buffer is what is currently on-screen, and is useful for screenshots.
     */
  ...
  private static BufferedImage createFlipped(BufferedImage image) {
    ...
    /*
     * Creates a compound affine transform, instead of just one, as we need to perform two transformations.
     * 
     * The first one is to scale the image to 100% width, and -100% height. (That's *negative* 100%.)
     */
    at.concatenate(AffineTransform.getScaleInstance(1, -1));
    /**
     * We then need to translate the image back up by it's height, as flipping it over moves it off the bottom of the canvas.
     */
